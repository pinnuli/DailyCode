...
public class Solution {
     ...
     * 此时百位对应的就是1，则共有a%10(最高两位0-30)次是包含100个连续点，当最高两位为31（即a=311），本次只对应局部点00~56，共b+1次，所有点加起来共有（a%10*100）+(b+1)，这些点百位对应为1；
     * 当i表示百位，且百位对应的数为0,如n=31056,i=100，则a=310,b=56，
     * 此时百位为1的次数有a/10=31（最高两位0~30）。
     * 综合以上三种情况，
     * 当百位对应0或>=2时，有(a+8)/10次包含所有100个点，
    ...
    public int numberof1Between1AndN(int n) {
        ...
        for (int i = 1; i <= n; i *= 10) {
            int a = n / i;
            int b = n % i;
            countOfOne = countOfOne + (a + 8) / 10 * i + (a % 10 == 1 ? 1 : 0) * (b + 1);
        }
        return countOfOne;
    }
     ...
     * @return
     */
    public int numberof1Between1AndNViolently(int n) {
        int countOfOne = 0;
        for (int i = 1; i <= n; i *= 10) {
            countOfOne += numberOf1(i);
        }
        return countOfOne;
    }
